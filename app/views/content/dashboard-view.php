...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['start_date'])) $_SESSION['dash_start'] = $_POST['start_date'];
    if(isset($_POST['end_date']))   $_SESSION['dash_end']   = $_POST['end_date'];
    if(isset($_POST['filtro_status_ods'])) $_SESSION['dash_status'] = $_POST['filtro_status_ods'];
    // 2. REDIRECCIÓN SEGURA (JS): 
    echo '<script> window.location.href = "' . APP_URL . $modulo_actual . '/"; </script>';
    exit();
}

if (!isset($_SESSION['dash_start'])) $_SESSION['dash_start'] = date('Y-m-d', strtotime('-3 weeks'));
if (!isset($_SESSION['dash_end']))   $_SESSION['dash_end']   = date('Y-m-d');

$start_date = $_SESSION['dash_start'];
$end_date   = $_SESSION['dash_end'];
$id_usuario = $_SESSION['id'];
$filtroActual = $_SESSION['dash_status'] ?? 'Todos';

// Lista completa de estados
$estadosPosibles = [
    'RECEPCION', 'DIAGNOSTICO', 'PRESUPUESTO', 'AUTORIZACION', 
    'REPARACION', 'REFACCIONES', 'STANDBY', 'LISTOE', 'ENTREGADO', 'CANCELADO'
];

// ---------------------------------------------------------
// 2. DATOS DEL GRÁFICO (SOLO ASESOR)
// ---------------------------------------------------------
...
$inner_colors = []; 
$inner_labels = []; 
$inner_ids = []; // <--- 1. AGREGADO: Array para guardar los IDs
$outer_groups = [];
...
foreach ($raw_data as $row) {
    ...
    $inner_ids[]    = $row['Idods']; // <--- 2. AGREGADO: Guardamos el ID
...
$js_in_ids = json_encode($inner_ids); // <--- 3. AGREGADO: Convertir IDs a JSON para JS
...
<div class="container is-fluid mb-6">
    ...
    <div class="columns is-variable is-4">
        ...
        <div class="column is-4-desktop is-12-tablet">
            <div class="dashboard-card">
                <div class="dashboard-card-header">
                    <span><i class="fas fa-chart-pie"></i> Mis Recepciones</span>
                </div>
                <div class="dashboard-card-body">
                    <div class="chart-wrapper">
                        <canvas id="odsChartMain"></canvas>
                    </div>
                    ...
                    <?php if ($total_ods == 0): ?>
                        <div class="notification is-light has-text-centered mt-3 is-size-7"><small>No tienes movimientos en este rango.</small></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="column is-8-desktop is-12-tablet">
            <div class="dashboard-card">
                <div class="dashboard-card-header">
                    <span><i class="fas fa-list-ul"></i> Mis ODS (Asesor)</span>
                ...
                <div class="dashboard-card-body p-0">
                    <div style="width: 100%;">
                        <?php
                            ...
                            echo $insOds->listarDashboardControlador($url[1], 10, $url[0], "", $filtroActual);
                        ?>
                    </div>
                </div>
            </div>
...
document.addEventListener("DOMContentLoaded", () => {
    <?php if ($total_ods > 0): ?>
        var ctx = document.getElementById('odsChartMain').getContext('2d');
        ...
        new Chart(ctx, {
            ...
            options: {
                ...
                // --- 4. AGREGADO: Evento Click ---
                onClick: (evt, elements) => {
                    if(elements.length > 0){
                        var idx = elements[0].index;
                        var datasetIdx = elements[0].datasetIndex;
                        // Dataset 0 es el anillo interno (las ODS individuales)
                        if(datasetIdx === 0){
                            var ids = <?php echo $js_in_ids; ?>;
                            var urlBase = '<?php echo APP_URL; ?>';
                            window.location.href = urlBase + 'odsView/' + ids[idx] + '/';
                        }
                    }
                },
                // ---------------------------------
                ...
                onHover: function(event, elements) {
                    if (elements && elements.length > 0) {
                        ...
                        // Cursor manita solo en anillo interno
                        ...
                        curText = "<?php echo $total_ods; ?>"; curSub  = "Total ODS"; curColor = "#1d4d80";
                    }
                    this.draw();
                },
